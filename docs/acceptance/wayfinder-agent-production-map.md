...
| Campaign Design Agent | Invokes `runCampaignDesignPipeline` when the exact Direct capability snapshot is available; its candidate passes the deterministic Direct Compiler before the complete pair is verified. Without that snapshot it produces only an explicit review-with-gaps result and does not claim compilation | No partial durable pair, Direct write, publication or spend authority |
