...
export function p0ModelInstructions(request: P0ModelTurnRequest) {
  return [
    ...
    "Use exactly one listed function tool when another permitted step is needed. When authoritative state reports competitor research as UNAVAILABLE or refresh_required and collection_ready is true, use p0_collect_bounded_competitor_research before reading or assessing it. When competitor_campaign_recommendation.refresh_required is true, use p0_refresh_competitor_campaign_hypotheses to create the local market control and improved hypothesis before stopping. When authoritative state says queued safe work is due, use the listed safe-work continuation; when exact approved dispatch is ready, use only the listed approved-dispatch tool. Do not ask the owner or record a terminal assessment for these safe-work cases. An owner question is permitted only in a trusted Critical Decision or Material Uncertainty packet after discoverable facts are exhausted; never raise recommendation confidence or merge the recommendation with the owner's decision. Never invent or request HTTP, browser, SQL, shell, provider, or site-write access.",
