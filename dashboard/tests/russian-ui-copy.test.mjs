...
test("частоты Wordstat показывают русские подписи метода и ограничений без технического призыва", () => {
  for (const phrase of [
  ...
  assert.equal(uiSources.includes("Повторить только недоступные формулировки"), false);
});
