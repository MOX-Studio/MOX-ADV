...
*План развивает существующий `dashboard/`. Он сохраняет внутреннюю доказуемость и безопасность, но полностью меняет owner-facing приоритет: сначала бизнес-смысл, затем решение человека, а техническая реализация остаётся внутри модуля.*
