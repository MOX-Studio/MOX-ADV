...
The keys live in `.claude/settings.json` (project) or `/Users/sviridov/Documents/MOX-ADV/.pi/settings.json` (global) — **not** in `.planning/config.json`:
...
See @/Users/sviridov/Documents/MOX-ADV/.pi/gsd-core/references/planner-guidance.md for Granularity Calibration table (Coarse/Standard/Fine plans-per-phase).
